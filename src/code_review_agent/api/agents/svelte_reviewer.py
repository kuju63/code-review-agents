...
from code_review_agent.agents.reviewers.svelte import SvelteReviewer
...
async def _run(task_id: str, data: dict, store: TaskStore, settings: Settings) -> None:
    ...
    try:
        ...
        reviewer = SvelteReviewer(config)
...
def svelte_reviewer_router(settings: Settings, store: TaskStore) -> APIRouter:
    ...
    async def get_agent_card() -> AgentCard:
        url = settings.resolve_agent_url(
            "svelte-reviewer", settings.agent_svelte_reviewer_url
        )
        return AgentCard(
            name="Svelte Reviewer",
            description="Reviews Svelte pull requests for reactivity (runes), event handling, snippets, styling, context, and legacy-feature migration. Returns no findings when the PR is not a Svelte project.",
            url=url,
            skills=[
                AgentSkill(
                    id="review_svelte_pr",
                    name="Review Svelte PR",
                    description="Performs a technical code review for a Svelte PR using GitHub MCP and the official svelte-core-bestpractices skill.",
