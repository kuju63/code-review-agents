"""A2A router exposing the Security Reviewer as an independently callable agent."""
...
from code_review_agent.agents.reviewers.security import SecurityReviewer
...
def security_reviewer_router(settings: Settings, store: TaskStore) -> APIRouter:
    """Build the A2A-compatible router for the Security Reviewer agent.

    Exposes the standard A2A endpoint trio: an agent card, task submission
    (which runs :class:`SecurityReviewer` in the background), and task polling.
    ...
    async def get_agent_card() -> AgentCard:
        url = settings.resolve_agent_url(
            "security-reviewer", settings.agent_security_reviewer_url
        )
        return AgentCard(
            name="Security Reviewer",
            description="Reviews pull requests for security vulnerabilities based on OWASP Top 10 and front-end attack patterns.",
            url=url,
            skills=[
                AgentSkill(
                    id="review_security",
