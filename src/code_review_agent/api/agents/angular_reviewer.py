"""A2A router exposing the Angular Reviewer as an independently callable agent."""
...
from code_review_agent.agents.reviewers.angular import AngularReviewer
...
async def _run(task_id: str, data: dict, store: TaskStore, settings: Settings) -> None:
    ...
    try:
        ...
        reviewer = AngularReviewer(config)
...
def angular_reviewer_router(settings: Settings, store: TaskStore) -> APIRouter:
    """Build the A2A-compatible router for the Angular Reviewer agent.

    Exposes the standard A2A endpoint trio: an agent card, task submission
    (which runs :class:`AngularReviewer` in the background), and task polling.
    ...
    async def get_agent_card() -> AgentCard:
        url = settings.resolve_agent_url(
            "angular-reviewer", settings.agent_angular_reviewer_url
        )
        return AgentCard(
            name="Angular Reviewer",
            description="Reviews Angular pull requests for component/service design, reactivity (signals), change detection, dependency injection, and template correctness.",
            url=url,
            skills=[
                AgentSkill(
                    id="review_angular_pr",
                    name="Review Angular PR",
                    description="Performs a technical code review for an Angular PR using GitHub MCP and the official angular-developer skill.",
