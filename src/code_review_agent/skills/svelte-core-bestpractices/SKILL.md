...
description: Reviews Svelte code using official Svelte guidance. Trigger when reviewing Svelte components (.svelte) or modules (.svelte.ts/.svelte.js), covering reactivity (runes), event handling, snippets, styling, context, async Svelte, and legacy-feature migration.
license: MIT
metadata:
  author: Svelte Contributors
  version: '1.0-review-adapted'
---

# Svelte Review Guidelines

1. Determine the project's Svelte version from repository manifests (for example `package.json` and `svelte.config.js`) before applying version-specific guidance. Do not recommend APIs that are unavailable in the detected version (for example, async expressions require Svelte 5.36 or higher).

2. Review only the supplied changes and repository context. Use the sections below as review criteria for correctness, reactivity soundness, maintainability, and performance.

3. Report concrete defects or material risks with evidence from the changed code. Do not request code generation, migrations, or command execution as part of the review.

4. Prefer consistency with the existing project when multiple valid Svelte patterns exist. Distinguish version-dependent modernization suggestions (for example runes migration) from correctness findings, and assign priority according to actual impact.
