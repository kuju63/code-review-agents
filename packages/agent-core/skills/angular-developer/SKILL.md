...
description: Reviews Angular code and architecture using official Angular guidance. Trigger when reviewing components, services, reactivity (signals, linkedSignal, resource), forms, dependency injection, routing, SSR, accessibility (ARIA), animations, styling, testing, or CLI configuration.
license: MIT
metadata:
...
1. Determine the project's Angular version from repository manifests before applying version-specific guidance. Do not recommend APIs that are unavailable in the detected version.

2. Review only the supplied changes and repository context. Use the references below as review criteria for correctness, maintainability, performance, and accessibility.

3. Report concrete defects or material risks with evidence from the changed code. Do not request project creation, code generation, migrations, or command execution as part of the review.

4. Prefer consistency with the existing project when multiple Angular patterns are valid. Distinguish version-dependent modernization suggestions from correctness findings and assign priority according to actual impact.
