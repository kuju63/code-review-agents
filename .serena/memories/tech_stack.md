...
- Testing: Vitest (`vitest.config.ts`), coverage provider v8, thresholds lines/functions/branches/statements each 75% (matches CONTRIBUTING.md's quality gate). `junit` reporter emits `./junit.xml` and `lcov` coverage emits `./coverage/lcov.info`; CI's `ci-check` job only runs `pnpm run test` (no Codecov upload step currently wired in `.github/workflows/ci.yaml`).
