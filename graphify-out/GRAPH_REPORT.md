...
## Corpus Check
- 521 files · ~316,022 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2537 nodes · 4553 edges · 181 communities (134 shown, 47 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 164 edges (avg confidence: 0.79)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `8aaf9c4f`
...
Cohesion: 0.07
Nodes (43): DEPENDENCY_FILENAMES, isDependencyFile(), isTargetFile(), TARGET_EXTENSIONS, TARGET_FILENAMES, buildSeededItem(), buildSeededItemFromFiles(), CATEGORIES (+35 more)
...
Cohesion: 0.19
Nodes (22): LLMReviewAgent, ReviewerClass, DefaultLLMFakeReviewer, NoMcpLLMFakeReviewer, ANGULAR_SOURCE_SUFFIXES, DETECTION_RULES, DetectionRule, getRegisteredReviewers() (+14 more)
...
Cohesion: 0.13
Nodes (19): send_discord_notification(), defaultGetCommitHash(), errorMessage(), execFileAsync, formatExecutedAt(), formatTsStr(), generateReport(), GenerateReportArgs (+11 more)
...
Cohesion: 0.10
Nodes (14): isInfraError(), StructuredOutputMissingError, getReviewerClasses(), METAFRAMEWORK_BASE, ReviewOrchestrator, ReviewOutcome, loadModules(), ReviewError (+6 more)

### Community 18 - "score-evaluation.ts"
Cohesion: 0.05
Nodes (63): RunDeps, defaultScore(), readJsonlRows(), readJsonl(), serializeRow(), directories, writeFileAtomic(), writeJsonAtomic() (+55 more)
...
Cohesion: 0.23
Nodes (12): listGithubOrgsRoute, listGithubPullRequestsRoute, listGithubRepositoriesRoute, unauthorizedResponse, upstreamGithubFailureResponse, validationErrorResponse, GithubOrgListResponseSchema, GithubOrgQuerySchema (+4 more)
...
### Community 23 - "build-gold-set.ts"
Cohesion: 0.07
Nodes (39): ApiGet, buildGoldItem(), BuildGoldItemDeps, CATEGORY_KEYWORDS, errorMessage(), extractLine(), FetchPrFiles, fetchReviewComments() (+31 more)

### Community 24 - "PRInfoCollector"
Cohesion: 0.22
Nodes (6): createMcpToolCaller(), extractHeadRef(), extractLabelNames(), extractToolTextBlocks(), isPlainRecord(), PRInfoCollector

### Community 25 - "pr-info-collector.ts"
Cohesion: 0.10
Nodes (21): ADR-0004, ReviewerConfig, createModelProvider(), CreateModelProviderOptions, ProviderType, mockedCreateOllama, mockedOpenAIModel, mockedVercelModel (+13 more)
...
Cohesion: 0.07
Nodes (8): A2APart, A2ATask, A2ATaskSchema, LeadEngineerTaskStore, OrchestratorTaskStore, TaskStore, InMemoryReviewerTaskStore, ReviewerTaskStore
...
### Community 30 - "app.ts"
Cohesion: 0.20
Nodes (11): createApp(), ADR-0012, errorBody(), failureStatus(), registerGithubRoutes(), buildTestApp(), REVIEW_REVIEWED_EXAMPLE, errorResponse() (+3 more)
...
Cohesion: 0.09
Nodes (16): annotatePatch(), buildPrompt(), composeSystemPrompt(), ReviewAgent, splitPatchLines(), context(), makePrInfo(), PlainFakeReviewer (+8 more)

### Community 34 - "a2a/request.model.ts"
Cohesion: 0.11
Nodes (26): A2ADataPart, A2ADataPartDiscriminatedSchema, A2ADataPartSchema, A2APartDiscriminatedSchema, A2APartSchema, A2ASendTaskRequestSchema, A2ATextPart, A2ATextPartDiscriminatedSchema (+18 more)

### Community 35 - "reviews/reviews.schema.ts"
Cohesion: 0.13
Nodes (24): REVIEW_ATTEMPT_CANCELED_EXAMPLE, REVIEW_ATTEMPT_QUEUED_EXAMPLE, REVIEW_CLOSED_EXAMPLE, REVIEW_COMMENT_EXAMPLE, REVIEW_DRAFT_EXAMPLE, REVIEW_LIST_EXAMPLE, REVIEW_REPORT_EXAMPLE, ApiVersionSchema (+16 more)

### Community 36 - "pr-info.service.ts"
Cohesion: 0.09
Nodes (20): app, settings, AgentCard, createPrInfoRoute(), CreatePrInfoRouteOptions, requestBody, A2AServerSettings, createPrInfoService() (+12 more)
...
### Community 61 - "manifest-detection.ts"
Cohesion: 0.41
Nodes (10): collectDirectPackageNames(), collectFields(), detectProjectTypeFromPackages(), extractDirectDependenciesFromPackageJson(), extractDirectDependenciesFromPackageLock(), extractDirectDependenciesFromPnpmLock(), isPlainRecord(), PACKAGE_PROJECT_TYPE_PRIORITY (+2 more)
...
### Community 72 - "ADR-0010: システム全体LLM同時実行上限（ProviderSemaphore+協調キャンセル採用）"
Cohesion: 0.31
...
Cohesion: 0.20
Nodes (10): 11. 関連ドキュメント, 2.1 Pydantic モデル（`src/code_review_agent/a2a/models.py`）, 2.2 TaskStore（`src/code_review_agent/a2a/task_store.py`）, 2. A2A プロトコル実装仕様, 7. `api/config.py` 実装仕様, 8. 依存関係の変更（`pyproject.toml`）, 9.1 `ReviewerConfig` の拡張, 9.2 `OpenAIModel` 生成部の変更（`LLMReviewAgent`, `PRInfoCollector`, `LeadEngineerAgent`） (+2 more)
...
### Community 85 - "each block key check"
Cohesion: 0.29
...
### Community 87 - "web-api/src/index.ts"
Cohesion: 0.24
...
Cohesion: 0.17
Nodes (18): RFC-3339, GithubApiClientOptions, githubApiGet(), GithubApiResult, GithubOrg, GithubPullRequest, GithubRepository, isRecord() (+10 more)
...
### Community 175 - "5. FastAPI アプリケーション構成"
Cohesion: 0.50
...
### Community 178 - "3. 全体アーキテクチャ"
Cohesion: 0.67
Nodes (3): 3.1 LangFlow ワークフロー → A2A API マッピング, 3.2 ディレクトリ構造（新規追加分）, 3. 全体アーキテクチャ
...
## Knowledge Gaps
- **812 isolated node(s):** `uvx`, `start-mcp-server`, `$schema`, `root`, `packages/**` (+807 more)
...
- **Why does `LeadEngineerReport` connect `models/index.ts` to `Lead Engineer Agent 設計`, `score-evaluation.ts`, `orchestrator.service.ts`, `lead-engineer.service.ts`?**
