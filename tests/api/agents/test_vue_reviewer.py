...
from code_review_agent.api.agents.vue_reviewer import vue_reviewer_router
from code_review_agent.api.config import Settings

_MOD = "code_review_agent.api.agents.vue_reviewer"
...
def _make_app(settings: Settings | None = None) -> tuple[FastAPI, TaskStore]:
    ...
    app.include_router(
        vue_reviewer_router(settings, store),
        prefix="/vue-reviewer",
    )
...
class TestAgentCard:
    def test_returns_agent_card(self) -> None:
        ...
        with TestClient(app) as client:
            resp = client.get("/vue-reviewer/.well-known/agent.json")
        assert resp.status_code == 200
        data = resp.json()
        assert data["name"] == "Vue Reviewer"
        assert "/vue-reviewer" in data["url"]
...
class TestSendTask:
    def test_returns_202_with_task_id(self) -> None:
        app, _ = _make_app()
        with TestClient(app) as client:
            resp = client.post("/vue-reviewer/tasks/send", json=_send_payload())
...
class TestGetTask:
    def test_returns_404_for_unknown_task_id(self) -> None:
        app, _ = _make_app()
        with TestClient(app) as client:
            resp = client.get("/vue-reviewer/tasks/nonexistent-id")
    ...
    async def test_background_task_completes(self) -> None:
        ...
        app, store = _make_app()
        with (
            patch(f"{_MOD}.VueReviewer") as mock_reviewer_cls,
