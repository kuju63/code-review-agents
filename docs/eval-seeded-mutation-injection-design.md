...
挿入位置 (`insert_idx`) は常に「diffの最初のhunkヘッダー直後」に固定されている。多くの場合これは
...
数えない) を積算した上で `base_line` に加算する必要がある。Phase 1.3.1.2 (挿入位置ヒューリスティック
