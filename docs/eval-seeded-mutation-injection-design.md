...
| フィールド | 型 | 意味 | 生成時の制約 |
|---|---|---|---|
| `mutated_patch` | 文字列 (unified diff全文) | 脆弱性パターンを注入した後のpatch全文 | 元patchに対し追加行 (`+`) の挿入のみで構成されること。削除行・コンテキスト行の書き換えを含んではならない (3.2.3で機械検証) |
...
| (B) `unidiff` 等の外部ライブラリを新規導入 | 却下 | Phase1で既に同等機能を実装済みであり、追加のメリットが薄い。新規依存はCVEスキャン対象を増やし([[feedback_container_security]]の方針とも整合しない) |
...
- `--seed` は「どの (file, rule) ペアを選ぶか」(`enumerate_combo_pool` → `rnd.shuffle`) の
  ...
  固定した成果物として扱うことで担保する。
- **モデル構成を変更した場合の扱い**: 3.2.6で導入する生成用モデル (`model_id`/`llm_base_url`)
...
(`load_dotenv()` は既に `build_seeded_set.py` 冒頭で行われている)。
