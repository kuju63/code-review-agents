...
- 7.4.1で採用した「前後空白の正規化」(`_normalize_diff_line_for_compare`)
  は、複数行にまたがるテンプレートリテラル/文字列リテラルの継続行では
  安全ではない。そうした行では先頭の空白そのものが文字列値の一部であり、
  LLMが再インデントだけを行った場合でも意味的な内容変更にあたるが、
  `_normalize_diff_line_for_compare`は本文をトークン列としてではなく
  文字列として比較するだけで、対象行が文字列リテラル内部かどうかを
  判定する手段を持たない(PR #133へのCopilotレビュー指摘)。
  この判定には複数行にまたがるバッククォート/引用符の対応状態を
  hunk全体、場合によってはファイル全体を通じて追跡する必要があり、
  現在の「diffテキストに対する軽量な字句比較」という設計方針の範囲を
  超える。7.4.1の緩和が救う対象(単純なコード行のインデント差・分類B)
  の方が、この見逃しうるケース(複数行文字列リテラルの継続行を狙い撃ちで
  再インデントするLLM出力)より遥かに一般的であるため、現時点では
  前後空白の正規化を維持し、この既知の限界として記録するにとどめる。
  対応が必要になった場合は、対象行が文字列リテラル内部かどうかを
  hunk単位で追跡する軽量パーサの導入を検討する。
...
実装は`render_seeded_item_with_generation()`・`build_seeded_items()`に
`max_attempts`引数 (関数レベルのデフォルトは`1`。既存の直接呼び出し元との
後方互換を保つための値であり、下記CLIのデフォルトとは独立) を追加し、
`generate_fn`の呼び出しから`passes_post_generation_checks`・
`recompute_injected_line()`までの一連の判定を、通過するかattemptsを
使い切るまで繰り返す。`main()`の新規CLIオプション`--llm-max-attempts`は
9.8の実測結果を受けデフォルト`3`とし、`build_seeded_items()`経由で渡す
(9.9参照)。
...
**残った5件のfallback**を個別に確認したところ、1件は3hunk・4397文字
(このrun全体で最大のpatch) と9.1の「大規模・多hunk patchでの内容欠落」
パターンに整合したが、残り2件は同一元patch (単一hunk・1323文字) に
異なるルールを適用したもので、他のLLM成功例と比べて特別大きい・複雑という
わけではなかった。つまりfallbackの一部は構造的な限界 (9.6) だが、一部は
9.8で確認する試行間分散そのものによるものであり、この内訳が9.8で複数回
測定を行う根拠になっている。
...
9.7・9.8の実測はいずれも`--model-id`・`--llm-max-attempts`をCLI引数で
明示指定した場合のものであり、実装直後の`--llm-max-attempts`の
デフォルトは`1` (リトライなし) のままだった。この状態では、
`evaluation/RUNBOOK.md`記載の通常手順 (オプション省略・`.env`の
`SEEDED_GEN_MODEL_ID=llama3.1:latest`のまま実行) では9.1・9.7の劣った
数値 (fallback率90%超) がそのまま再現され、「目標を達成した」という
9.8の結論はデフォルト状態には及ばない。この乖離をユーザーに確認した結果、
以下の分担で対応する。

- **`--llm-max-attempts`のCLIデフォルトを`3`に変更する** (本コミットで対応)。
  `render_seeded_item_with_generation()`・`build_seeded_items()`関数自体の
  デフォルト (`1`) は、既存の直接呼び出し元 (テストなど) との後方互換の
  ために変更しない。CLIのデフォルトのみを9.8の実測値に合わせる。
- **`.env`の`SEEDED_GEN_MODEL_ID`を`Ornith-1.0-35B`に更新することは、
  運用者 (ユーザー) が手動で行う**。`.env`は秘匿情報を含み得るファイルで
  Gitignore対象かつAgentによる直接編集の対象外であること、また3.2.4が
  「モデル変更は運用者の明示的な操作」と定めていることの両方に整合する。
  24.7GBというモデルサイズ・1呼び出しあたりのレイテンシ増加という
  コストをユーザー自身が把握した上で切り替える必要がある。

`--llm-max-attempts`のデフォルト変更単独では、`--model-id`が未指定のまま
`.env`側が`llama3.1:latest`を指し続ける限り、9.1・9.2で確認した通り
fallback率30%未満を達成しない。デフォルト実行でも目標を再現するには、
本節の2つの対応が両方揃う必要がある。
