...
MCP ツール定義＋会話履歴＋フェッチ結果の蓄積がモデルのコンテキスト上限を超えることが判明した。

Seeded 評価は `evaluate_seeded_item()` で独自に patch を差し込む設計のため同じ問題が起きず、
Gold と Seeded の評価パスが非対称になっていた（Gold のみが findings=0 になる構造的問題）。

| 評価種別 | プロンプト長 | patch 含有 | MCP fetch |
|---|---|---|---|
...
| 判断 | 内容 |
|---|---|
| patch フィールドの取得元 | GitHub `get_files` レスポンスに `patch` フィールドが既に含まれる。`_read_changed_files()` が返す raw dict からそのまま使用（追加 API 呼び出しは不要）。 |
| 同梱の条件 | diff 総量が閾値（`patch_total_char_limit` chars・`patch_max_files` ファイル）以内の場合のみ全ファイルに同梱する。 |
| 超過時の挙動 | 全ファイルを `patch=None` にフォールバックし WARNING ログを出す。レビュアーは従来どおり MCP フェッチにフォールバックする。 |
| 部分的な同梱の採否 | 却下。ファイルごとに include/None を混在させると、省いたファイルだけ MCP フェッチが起きて context が予測不能に膨らむ。一括 include か一括 None のどちらかに統一する。 |

**デフォルト閾値**:
- `patch_total_char_limit = 30_000` chars（評価実績の Seeded: 14,800 chars の約2倍）
- `patch_max_files = 30` ファイル
...
- **gemma 固有の失敗モード**: ツール名ハルシネーション・reasoning ループは patch 同梱とは独立した問題。
  適切なモデル選択で対処する（方針2 の対象外）。
