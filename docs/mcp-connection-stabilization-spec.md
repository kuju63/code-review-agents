...
で確定した**方式**を、TDD実装にそのまま着手できる粒度まで落とし込む外部設計ドキュメントである。
両ADRは「何を選んだか・なぜか」を確定させるものであり、「誰が・どの順番で・どの責務を持って動くか」という
コンポーネント間のインターフェース・処理シーケンス・エラー方針は未確定のまま残っている。本ドキュメントは
その残りを確定させる。
...
(2)が本ドキュメントで新たに追加される経路である。(2)は**`start()`を直接呼んではならない**という制約がある:
strandsの`MCPClient`は「起動済みかどうか」を示す内部状態を`load_tools()`の呼び出し時にのみ更新する。
仮にオーケストレータが低レベルの`start()`だけを呼んで済ませると、後から各レビュアーの`Agent`が
...
(2)・(3)はいずれも`Agent`/`ToolProvider`のライフサイクル管理下にあるため、リトライは`MCPClient`の起動処理
そのもの((1)(3)は`start()`、(2)は`load_tools()`)を包む形で適用する。(1)から(3)までの3経路すべてに同じ
バックオフパラメータを適用し、経路によって挙動を変えない。

### 3.3 リトライ対象の例外型(経路により異なる)

`MCPClient.load_tools()`は内部で`start()`を呼び、失敗時は原因例外を`ToolProviderException`でラップし直す
(strands `mcp_client.py`)。そのため、経路によってリトライがハンドルすべき例外の型が異なる:
...
リトライ実装(TDD着手時)はこの2つの例外型をそれぞれの呼び出し箇所で正しく捕捉する必要がある。原因種別
(一過性/非一過性)による分岐は[ADR-0003](adr/0003-github-mcp-startup-retry-strategy.md)の決定通り行わず、
最大試行回数を小さく抑えることで検知遅延を許容範囲にとどめる方針を維持する。
...
| ファイル | 変更の性質 |
|---|---|
| [src/code_review_agent/tools/github_mcp.py](../src/code_review_agent/tools/github_mcp.py) | 変更なし(既存の`create_github_mcp_client`をそのまま利用) |
| [src/code_review_agent/agents/review_orchestrator.py](../src/code_review_agent/agents/review_orchestrator.py) | 共有クライアントの生成・初回起動(リトライ込み)・参照登録/解放を追加 |
...
| [src/code_review_agent/agents/pr_info_collector.py](../src/code_review_agent/agents/pr_info_collector.py) | 起動処理(経路(1))にリトライを追加。それ以外は変更なし |
