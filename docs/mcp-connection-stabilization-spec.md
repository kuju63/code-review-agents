...
| (3) 並列レビュー段のフォールバック(共有接続が使えない単体利用時) | `MCPClient.load_tools()`(非同期・`Agent`が`ToolProvider`として呼ぶ。内部で`start()`) | `LLMReviewAgent.review()`(`src/code_review_agent/agents/base_reviewer.py`)、共有接続が渡されなかった場合 |
...
- 実装手段は `tenacity`(既存の間接依存を直接依存へ昇格。3.6節参照)。
...
`start()`をリトライ機構でラップする形で実装する想定」と述べている通り)。(1)のように`start()`を直接
呼ぶ経路はもちろん、(2)(3)のように`Agent`/オーケストレータが`load_tools()`を経由する経路も、その内部
呼び出しが同じ(リトライ機構でラップ済みの)`start()`である以上、自動的にリトライの恩恵を受ける。
経路ごとに個別の実装は不要であり、
...
`tenacity` を `pyproject.toml` の直接依存として追加する。現時点では`strands-agents`ではなく
`strands-agents-tools`(`pyproject.toml`で直接依存として指定済み、`>=0.8.2`)が`tenacity`に依存しており、
`uv.lock`上ではその間接依存として既に解決済みである。そのため通常はバージョン変更を伴わない見込みだが、
