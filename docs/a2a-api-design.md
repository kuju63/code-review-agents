  ...
  "skills": [
    {
      ...
      "inputSchema": {
        ...
        "properties": {
          ...
          "github_token": { "type": "string",  "description": "GitHub PAT（classic PAT: repo スコープ、または fine-grained PAT: Contents/Pull requests の Read 権限）" },
...
GitHub PAT（classic PAT の `repo` スコープ、または fine-grained PAT の Contents / Pull requests Read 権限）が必要ですが、**渡し方には 2 つの選択肢**があります。

| 方式 | 説明 | 向いている用途 |
|---|---|---|
| **サーバー環境変数方式** | `GITHUB_TOKEN` 環境変数をサーバープロセスに設定し、`Settings` が読み取って各 Agent に注入する | CI / 自動化、共有サービスとして運用する場合 |
| **クライアント提供方式** | 各リクエストの `github_token` フィールドでトークンを渡す（現行 API 仕様） | ユーザーごとに異なるトークンを使う場合、ローカル開発 |

運用方針に応じてどちらかを選択してください。両方実装する場合は「リクエスト値 → 環境変数」の優先順位を `config.py` で定義します。
