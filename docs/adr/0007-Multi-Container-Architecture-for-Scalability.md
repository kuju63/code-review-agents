...
API Gateway（WebUI/CLI用）と、メッセージキュー（Redis/RabbitMQ等）を介した Worker コンテナに分離する構成。**最大N並列は、Workerの台数によらずシステム全体でのLLM同時実行数の上限を表す（Worker 1台あたりN並列ではない）。** 上限をどの機構（Worker側の合算設定、共有ロック、キュー側の同時取得数制御など）で実現するかは、本ADRでは決定せず後続ADRで定める。
...
5. **利点**: Worker が何台であっても、システム全体のLLM同時実行数を固定上限（例: 最大N並列）に制限することで、LLMへの流量を正確に制御できる。

| メリット | デメリット |
| --- | --- |
| **厳密な流量制御**: メッセージキューを介することで、Workerの台数によらずLLMの同時実行数をシステム全体で正確に制御（例: 最大N並列）でき、バックプレッシャーを適切に管理できる。 | **システムの複雑性増大**: メッセージキューの運用、冪等性の確保、非同期処理のハンドリングが必要。 |
...
| **耐障害性向上の可能性**: 永続化されたQueueと後続ADR（Issue #365）で定義する配信契約を採用した場合、Workerがクラッシュしてもジョブを保持し再試行できる可能性がある。ジョブが実際に保持されるか（Queue実装方式・永続化設定・ACK時点に依存）、再試行が可能かは、いずれもIssue #365の決定に依存し、本ADRでは保証しない。 | |
...
**根拠:**
1. **流量制御の実現**: 案Bを採用することで、メッセージキューをバッファとして利用し、LLMの処理能力に応じた固定の並列数でWorkerを稼働させることができる。これにより、**受付**（GWがクライアントから要求を受け取る量）を同時2リクエスト程度の制約から解放しつつ、**実行**（LLMを実際に呼び出す並列数）はシステム全体で定めた上限を超えないよう分離して制御できる。
...
- **流量制御の詳細設計**: Queue実装方式の比較、システム全体のLLM同時実行上限の実現方式、timeout/cancellation/stragglerの扱い、Workerクラッシュ後の配信契約（delivery semantics・retry・dead-letter・冪等性）、再起動時のジョブ回復は本ADRでは決定せず、Issue #345 のフォローアップである Issue #365 で `docs/adr/0009-localllm-review-flow-control.md` として決定する。
