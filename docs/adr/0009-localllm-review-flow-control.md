...
- Queue上限・backpressure・過負荷時のAPIレスポンス（#366 受け入れ条件に対応）:
  ...
  - **enqueue受付の冪等性**:
    ...
    - **同一キーの再送**: 容量判定や新規enqueueを行わず、既存レコードとpayload hashを比較する。
      ...
      これは既存ジョブの参照であり `N_queue` を消費しない。hashが異なる場合はキーの誤再利用
      として `409 Conflict` を返し、新規enqueueを行わず、既存のtaskIdもレスポンスへ含めない。
      既存ジョブの状態・payload・保持期限は変更しない。
