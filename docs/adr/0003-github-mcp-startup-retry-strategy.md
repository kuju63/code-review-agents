...
「1. リトライを入れる」「2. MCPクライアントの使い回し」の2方針までは合意済みである。
...
調査の結果、当初のIssue記述だけでは見えていなかった2点を追加で発見した。

1. `tenacity`は`strands-agents`の間接依存として既に`uv.lock`に解決済みであり、
   新規パッケージ追加コストなしで直接依存に昇格できる。
2. `base_reviewer.py`経由(strands `Agent`がMCPClientを`ToolProvider`として所有する経路)の
   起動失敗は`ToolProviderException`にラップされるが、この例外は現行の`INFRA_EXCEPTIONS`
   (`src/code_review_agent/agents/exceptions.py`、Issue #56で確定した設計)に含まれておらず、
   リトライを尽くした後の最終失敗が`review_orchestrator.py`で通常の`ReviewError`(部分失敗)に
   格下げされてしまう既存の設計矛盾がある。リトライを入れる作業と不可分なので、本ADRの
   スコープに含める。
...
```python
# github_mcp.py
class _RetryingMCPClient(MCPClient):
    def start(self) -> "MCPClient":
        retryer = Retrying(
            stop=stop_after_attempt(self._max_attempts),
            # wait_random_exponential: 指数バックオフにジッターを加え、同時に失敗した
            # 複数セッションの再試行タイミングが同期しないようにする(方式1参照)。
            wait=wait_random_exponential(multiplier=self._backoff_seconds, max=8),
            retry=retry_if_exception_type(MCPClientInitializationError),
            reraise=True,
        )
        return retryer(super().start)
```

`create_github_mcp_client`がこのサブクラスを返すよう変更するだけで、
`pr_info_collector.py`/`base_reviewer.py`双方の呼び出し元は無変更で救済される。
