...
- Date: 2026-07-12
- Related: Issue #115, [docs/plan/review-orchestrator-infra-exception-propagation-spec.md](../plan/review-orchestrator-infra-exception-propagation-spec.md), [docs/plan/github-mcp-streamable-http-migration-spec.md](../plan/github-mcp-streamable-http-migration-spec.md)
...
- `mcp_startup_retry_attempts: int = 3`(tenacityの`stop_after_attempt`にそのまま渡す値。
  初回試行を含めた回数であり、再試行[リトライ]自体は最大2回である点に注意)
...
2. リトライ間隔は指数バックオフ+ジッターとし、最大試行回数3回(初回の実行を含む。
   tenacityの`stop_after_attempt(3)`は初回試行を含めた回数を指定する仕様であり、
   実質的な再試行(リトライ)は最大2回)・基準待機1秒程度とする。
...
- `pyproject.toml`に`tenacity`を直接依存として追加する必要がある。現行の`uv.lock`に
  既にバージョン解決済みで含まれているため、通常はバージョン変更を伴わない見込みだが、
  `pyproject.toml`に付与する制約(バージョン範囲等)次第では解決結果が変わり得る点に留意する。
