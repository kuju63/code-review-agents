...
設計（現行TS実装）: [docs/a2a-api-design.md](../a2a-api-design.md)

本ドキュメントは、A2A APIをPython/FastAPIで実装した時点の設計全文（§2〜§12、コード例含む）と、
その後TypeScript/Zodへ移行した際の実装計画（§13）をあわせて記録する歴史的資料である。
現行TS実装（`packages/a2a-server`）の設計は上記リンク先が正。本ドキュメントの§2〜§12は
Pydanticモデル・FastAPIルーティング等、現存しないPython実装のコード例を含むが、
プロトコル設計・セキュリティ設計の意図自体は現行TS実装にも引き継がれている
（対応関係は[docs/a2a-api-design.md](../a2a-api-design.md)を参照）。

実装計画: [docs/plan/a2a-api-implementation.md](a2a-api-implementation.md)（Python版実装のPhase分割）。
...
前段に配置し HTTPS を使用することを必須とする（[docs/a2a-api-design.md](../a2a-api-design.md) §1.3に注記済み）。
...
**現在の方針**: モノリス構成（[docs/a2a-api-design.md](../a2a-api-design.md) §1.3）では許容範囲。
